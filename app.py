...
def parse_log_content(log_content):
    ...
    for row in reader:
        ...
        try:
            long_start = (
                float(row[col_long_start]) if col_long_start is not None else None
            )
            long_end = float(row[col_long_end]) if col_long_end is not None else None
            short_start = (
                float(row[col_short_start])
                if col_short_start is not None and row[col_short_start]
                else None
            )
            short_end = (
                float(row[col_short_end])
                if col_short_end is not None and row[col_short_end]
                else None
            )
...
def select_directory():
    ...
    for f_path in current_playlist:
        ...
        files_with_transcription_info.append(
            {"name": f_name, "url": file_url, "error_segments": error_segments}
        )
...
def upload_log():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "message": f"Error parsing uploaded log file: {str(e)}"}
        )
...
def load_log_from_path():
    ...
    if not os.path.exists(log_file_path):
        return jsonify(
            {
                "success": False,
                "message": f"Log file not found at path: {log_file_path}",
            }
        )
    ...
    if not os.path.isfile(log_file_path):
        return jsonify(
            {
                "success": False,
                "message": f"Provided path is not a file: {log_file_path}",
            }
        )
    ...
    try:
        ...
        return jsonify(
            {
                "success": False,
                "message": f"Error loading or parsing log file: {str(e)}",
            }
        )
...
def get_csv_data():
    ...
    return jsonify(
        {"success": True, "data": csv_error_data, "count": len(csv_error_data)}
    )
...
def save_label():
    ...
    try:
        ...
        if not all(
            key in data for key in ["record_id", "start_time", "end_time", "audio_file"]
        ):
...
def download_labels():
    ...
    if os.path.exists(output_file):
        return send_file(
            output_file, as_attachment=True, download_name="labeled-segments.csv"
        )
    else:
...
def view_labels():
    ...
    if not os.path.exists(output_file):
        return jsonify(
            {"success": False, "message": "No labeled segments file found", "data": []}
        )
    ...
    try:
        if os.path.exists(output_file):
            ...
            return jsonify(
                {
                    "success": True,
                    "message": "All labeled segments deleted successfully",
                }
            )
        else:
            return jsonify(
                {
                    "success": False,
                    "message": "No labeled segments file found to delete",
                }
            )
    except Exception as e:
        return jsonify(
            {"success": False, "message": f"Error deleting labels file: {str(e)}"}
        )
